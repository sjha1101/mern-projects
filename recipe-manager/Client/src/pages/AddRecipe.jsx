...
function AddRecipe() {
    const [form, setForm] = useState({
        name: "", category: "", ingredients: "", description: "", cookingTime: "", image: null
    });
    ...
    const handleSubmit = async e => {
        ...
        try {
            const res = await fetch(`${API}/api/add`, { method: "POST", body: formData });
            const data = await res.json();

            if (res.ok) window.location.href = "/HomePage";
            else alert(data.message || "Failed to add recipe");
        } catch (err) {
            console.error(err);
                        ...
                        <div className="row g-4">
                            ...
                            <div className="col-md-6">
                                ...
                                <select name="category" className="form-select" onChange={handleChange} required>
                                    <option value="">Select</option><option>Breakfast</option><option>Lunch</option><option>Dinner</option>
                                </select>
                            </div>
                            <div className="col-md-6">
                                <label className="form-label">Ingredients</label>
                                <textarea name="ingredients" className="form-control" rows="3" onChange={handleChange} required></textarea>
                            </div>
                            <div className="col-md-6">
                            ...
                            <div className="col-md-6">
                                ...
                                <input type="number" name="cookingTime" className="form-control" onChange={handleChange} required />
                            </div>
                        </div>
                        <div className="text-center mt-4"><button type="submit" className="btn btn-custom">Submit</button></div>
                    </form>
                </div>
            </div>
        </div>
